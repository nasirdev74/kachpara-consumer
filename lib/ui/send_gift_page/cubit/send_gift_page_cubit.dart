...
part 'send_gift_page_state.dart';

part 'send_gift_page_cubit.freezed.dart';

@injectable
class SendGiftPageCubit extends Cubit<SendGiftPageState> {
  SendGiftPageCubit(
    this._promotionUseCase,
    this._userUseCase,
    this._storeUsecase,
    this._orderUseCase,
  ) : super(const SendGiftPageState());
  ...
  Future<void> initialize({required String storeId, required String promotionId, required String userId}) async {
    emit(state.copyWith(sendGiftPageStatus: Status.loading));
    ...
    emit(state.copyWith(sendGiftPageStatus: Status.success, store: store, freePromotion: freePromotion, user: user));
  }
  ...
  Future<void> checkout() async {
    ...
    emit(state.copyWith(sendGiftPageStatus: Status.loading, sendGiftPageError: null));

    if (selectedPaymentMethod == null) {
      emit(state.copyWith(
        sendGiftPageStatus: Status.failed,
        sendGiftPageError: "Kredi Kartı Bilgisi Giriniz.",
      ));
      return;
    }
    ...
    result.when(success: (data) {
      emit(state.copyWith(sendGiftPageStatus: Status.success, orderId: data));
    }, error: (error) {
      emit(state.copyWith(sendGiftPageStatus: Status.failed, sendGiftPageError: error));
    });
  }
}
